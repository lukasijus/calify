 ...
 */
export function filterByPeriod<T extends { at: string }>(entries: readonly T[], period: Period, anchorAt?: string): T[] {
  const sorted = sortByTime(entries);
  if (period === "All" || sorted.length === 0) return sorted;
  const anchor = Date.parse(anchorAt ?? sorted[sorted.length - 1].at);
